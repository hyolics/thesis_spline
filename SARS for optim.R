  ...
  while((nrow(data_selection)>1)&(delta<delta_0)){
    ...
    for(i in 1:nrow(data_selection)){
      ...
      if(nrow(J)>5){
        ...
        if(gamma>C){
          # tmp <- last(optim(data_selection$x[i], fn=gradient, method = "CG")$par)
          # tmp <- J$x[which.min(abs(tmp-J$x))]
    ...
    if (is.null(Sp[[p]][[2]])==FALSE){
      fit <- lm(y.spline~bs(x, degree=3, knots=Sp[[p]][[2]]), data)
      k <- length(Sp[[p]][[2]])
      sigma_hat <- median(abs(data$y.spline[-1]-data$y.spline[-length(data$y.spline)]))/(0.6745*sqrt(2))
      r <- sum(fit$residuals^2)+ 2*(k+4)*sigma_hat/n
      bic[[p]] <- n*log(r/n)+k*log(n)
   ...
   #bic
   #Sp[[which.min(bic)]]
   result <- optim(Sp[[which.min(bic)]][[2]], fn=rss, method = "CG")$par
   return(result)
}

SARS_opt(data, 5)
#fit
fit_SARS_opt <- function(data_e0, data, C){
  plot(data, lwd=2, main="SARS. with opt")
  lines(data_e0, lwd=2)
  abline(v=knot, lty=3)
  
  result <- SARS_opt(data, C)
